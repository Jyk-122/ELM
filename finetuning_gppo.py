...
def get_args_parser():
    parser = argparse.ArgumentParser("ELM-GPPO finetuing", add_help=False)
    ...
    parser.add_argument("--gppo_update_freq", type=int, default=10, help="")
    parser.add_argument("--gppo_update_iters", type=int, default=2, help="")
    parser.add_argument("--gppo_clip", type=float, default=0.2, help="")
...
def train_one_epoch(
    ...
    for data_iter_step, (example, label, example_mask, label_mask) in enumerate(
        ...
        if data_iter_step % accum_iter == 0:
            lr_sched.adjust_learning_rate(optimizer_policy, data_iter_step / len(data_loader) + epoch, args)
            lr_sched.adjust_learning_rate(optimizer_reward, data_iter_step / len(data_loader) + epoch, args)
            # lambda_lmloss, lambda_distill = lr_sched.adjust_lambda(data_iter_step / len(data_loader) + epoch, args)
            lambda_lmloss, lambda_distill = args.lambda_lmloss, args.lambda_distill

        action, log_prob = model.module.get_action()
        reward, loss_dict = model(action, example, label, example_mask, label_mask)
        ...
        if data_iter_step > 0 and data_iter_step % args.gppo_update_freq == 0:
            batch_action = torch.tensor(list_action).to(device).detach()
            batch_reward = torch.tensor(list_reward).to(device).detach()
            batch_log_prob = torch.tensor(list_log_prob).to(device).detach()
            
            advantages = (batch_reward - batch_reward.mean()) / (batch_reward.std() + 1e-8)
            
            for _ in range(args.gppo_update_iters):
                optimizer_policy.zero_grad()
                
                action_logits = model.module.policy_model()
                ...
                surr2 = torch.clamp(ratios, 1 - args.gppo_clip, 1 + args.gppo_clip) * advantages
                
                policy_loss = -torch.min(surr1, surr2).mean()
                p_loss_value = policy_loss.item()
        ...
        lr = optimizer_policy.param_groups[0]["lr"]
        metric_logger.update(closs=c_loss_value)
        metric_logger.update(dloss=d_loss_value)
        metric_logger.update(ploss=p_loss_value)
        ...
        if log_writer is not None and (data_iter_step + 1) % accum_iter == 0:
            ...
            log_writer.add_scalar("Train/p_loss", p_loss_value_reduce, step)
...
def main(args):
    ...
    for name, p in model_without_ddp.named_parameters():
        if 'policy' in name:
            param_group_policy.append(p)
        if 'reward' in name:
            param_group_reward.append(p)
            
    optimizer_policy = torch.optim.AdamW(param_group_policy, lr=args.lr, betas=(0.9, 0.95))
    optimizer_reward = torch.optim.AdamW(param_group_reward, lr=args.lr, betas=(0.9, 0.95))
    loss_scaler = NativeScaler()

    misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer_list=[optimizer_policy, optimizer_reward], loss_scaler=loss_scaler)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, data_loader_train, optimizer_policy, optimizer_reward, device, epoch, loss_scaler, log_writer=log_writer, args=args
        )
        ...
        if args.ckpt_dir and ((epoch + 1) % (args.save_freq) == 0 or epoch + 1 == args.epochs):
            misc.save_model(
                ...
                optimizer_list=[optimizer_policy, optimizer_reward],
