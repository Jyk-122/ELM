...
class FixedPointLayer(nn.Module):
    def __init__(self, args: ModelArgs):
        """
        Initialize a FixedPointLayer as a Transformer Block.
        ...
        Attributes:
            ...
            attention_norm (RMSNorm): Layer normalization for attention output.
            ffn_norm (RMSNorm): Layer normalization for feedforward output.
            proj: Adaptor module for combining iteration point and conditional input.
    ...
    def forward_step(
        ...
        """
        Perform a single forward pass step through the FixedPointLayer.
        ...
        z = self.proj(z)
        z = z + self.attention(self.attention_norm(z), start_pos, freqs_cis, mask)
    ...
    def forward(
        ...
        """
        Perform forward pass training of the FixedPointLayer.
        ...
        Args:
            ...
            freqs_cis (torch.Tensor): Precomputed cosine and sine frequencies.
            mask (torch.Tensor, optional): Causal masking tensor for attention. Defaults to None.
            iter_info (Dict): iteration information of Stochastic Jacobian-Free Backpropagation.

        Returns:
            torch.Tensor: Output tensor after fixed-point solving.

        """
        z = torch.zeros_like(x)
        
        with nullcontext():
            fixed_point_wo_grad_min_iters = iter_info["fixed_point_wo_grad_min_iters"]
            fixed_point_wo_grad_max_iters = iter_info["fixed_point_wo_grad_max_iters"]
            num_iterations_wo_grad = random.randint(fixed_point_wo_grad_min_iters, fixed_point_wo_grad_max_iters)
            for _ in range(num_iterations_wo_grad):
                z = self.forward_step(z, x, start_pos, freqs_cis, mask)
            z = z.detach()
        
        fixed_point_with_grad_min_iters = iter_info["fixed_point_with_grad_min_iters"]
        fixed_point_with_grad_max_iters = iter_info["fixed_point_with_grad_max_iters"]
        num_iterations_with_grad = random.randint(fixed_point_with_grad_min_iters, fixed_point_with_grad_max_iters)
        for _ in range(num_iterations_with_grad):
            z = self.forward_step(z, x, start_pos, freqs_cis, mask)
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        """
        Initialize an Equilibrium Language Model for training.
